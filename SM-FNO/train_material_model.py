...
argparser.add_argument("--continue_train", type=int, help = "if == 1, then continue from checkpoint", default=0)
...
from JAX_SM_FNO_material_conv import FNO_multimodal_2d
from simulation_dataset_JAX_material_only import SimulationDataset
...
def H_to_bc(Hz_R, Hz_I, dL, omega, yeex, yeey, EPSILON_0 = EPSILON_0):
    Hz = Hz_R + 1j*Hz_I
    top_bc = (Hz[:,0,1:-1]-Hz[:,1,1:-1])+1j*2*jnp.pi*jnp.sqrt(yeex[:,1,:])/wavelength*dL*1/2*(Hz[:,0,1:-1]+Hz[:,1,1:-1])
    bottom_bc = (Hz[:,-1,1:-1]-Hz[:,-2,1:-1])+1j*2*jnp.pi*jnp.sqrt(yeex[:,-1,:])/wavelength*dL*1/2*(Hz[:,-1,1:-1]+Hz[:,-2,1:-1])
    left_bc = (Hz[:,1:-1,0]-Hz[:,1:-1,1])+1j*2*jnp.pi*jnp.sqrt(yeey[:,:,1])/wavelength*dL*1/2*(Hz[:,1:-1,0]+Hz[:,1:-1,1])
    right_bc = (Hz[:,1:-1,-1]-Hz[:,1:-1,-2])+1j*2*jnp.pi*jnp.sqrt(yeey[:,:,-1])/wavelength*dL*1/2*(Hz[:,1:-1,-1]+Hz[:,1:-1,-2])
...
def main(args,seed):
    ...
    if args.continue_train:
        ...
        opt_state = checkpoint['opt_state'], # this seems to be useless? (because we need to do init again below)
    ...
    def compute_loss_only(model, yeex, yeey, top_bc, bottom_bc, left_bc, right_bc, y):
        pred_y, bc, grid = eqx.filter_vmap(model.eval_forward)(yeex, yeey, top_bc, bottom_bc, left_bc, right_bc)

        data_loss = MAE_loss(pred_y, y)
        FD_Hy = H_to_H(pred_y[:, :, :, 0], pred_y[:, :, :, 1], dL, omega, yeex[:,:,1:-1], yeey[:,1:-1,:])
    ...
    def compute_loss_and_grad(model, yeex, yeey, top_bc, bottom_bc, left_bc, right_bc, y, dL, omega, reg_norm, data_weight, inner_weight, bc_weight):
        pred_y = eqx.filter_vmap(model)(yeex, yeey, top_bc, bottom_bc, left_bc, right_bc)
        ...
        bc_gt = H_to_bc(y[:,:,:,0], y[:,:,:,1],dL, omega, yeex[:,:,1:-1], yeey[:,1:-1,:])
        bc_pred = H_to_bc(pred_y[:,:,:,0], pred_y[:,:,:,1], dL,omega, yeex[:,:,1:-1], yeey[:,1:-1,:])
    ...
    def train_step(rep_models, yeex, yeey, top_bc, bottom_bc, left_bc, right_bc, y, replicated_opt_state, dL, omega, reg_norm, data_weight, inner_weight, bc_weight):
        (loss, aux), grads = compute_loss_and_grad(rep_models, yeex, yeey, top_bc, bottom_bc, left_bc, right_bc, y, dL, omega, reg_norm, data_weight, inner_weight, bc_weight)
    ...
    @eqx.filter_pmap(axis_name='batch')
    def eval_step(rep_models, yeex, yeey, top_bc, bottom_bc, left_bc, right_bc, y):
        loss, reg_inner, pred_y, bc, grid = compute_loss_only(rep_models, yeex, yeey, top_bc, bottom_bc, left_bc, right_bc, y)
    ...
    for step in range(start_epoch, args.epoch):
        ...
        for idx, sample_batched in enumerate(train_loader):
            y_batch_train, yeex_batch_train, yeey_batch_train, top_bc_train, bottom_bc_train, left_bc_train, right_bc_train = jnp.asarray(sample_batched['field']), jnp.asarray(sample_batched['yeex']), jnp.asarray(sample_batched['yeey']), jnp.asarray(sample_batched['top_bc']), jnp.asarray(sample_batched['bottom_bc']), jnp.asarray(sample_batched['left_bc']), jnp.asarray(sample_batched['right_bc'])
            ...
            yeex_batch_train = yeex_batch_train.reshape(args.num_GPUs,int(args.batch_size/args.num_GPUs),yeex_batch_train.shape[1],yeex_batch_train.shape[2])
            yeey_batch_train = yeey_batch_train.reshape(args.num_GPUs,int(args.batch_size/args.num_GPUs),yeey_batch_train.shape[1],yeey_batch_train.shape[2])
            ...
            loss, replicated_models, replicated_opt_state, aux = train_step(replicated_models, yeex_batch_train, yeey_batch_train, top_bc_train, bottom_bc_train, left_bc_train, right_bc_train, y_batch_train, replicated_opt_state, dL, omega, reg_norm, args.data_weight, args.inner_weight, args.bc_weight)

            data_loss, inner_loss, bc_loss = aux
            running_average_data_loss = 0.99*running_average_data_loss+0.01*jnp.mean(data_loss).item()
            running_average_inner_residue = 0.99*running_average_inner_residue+0.01*jnp.mean(inner_loss).item()
            if idx%100 ==0:
                print('Epoch [{}/{}], Step [{}/{}], data Loss: {:.4f}, inner Loss: {:.4f}, bc loss: {:.4f}'.format(
                    ...
                    idx + 1,
                    total_step,
                    jnp.mean(data_loss).item(),
                    jnp.mean(inner_loss).item(),
                    jnp.mean(bc_loss).item()
                    )
                )
        ...
        for idx, sample_batched in enumerate(test_loader):
            y_batch_test, yeex_batch_test, yeey_batch_test, top_bc_test, bottom_bc_test, left_bc_test, right_bc_test = jnp.asarray(sample_batched['field']), jnp.asarray(sample_batched['yeex']), jnp.asarray(sample_batched['yeey']), jnp.asarray(sample_batched['top_bc']), jnp.asarray(sample_batched['bottom_bc']), jnp.asarray(sample_batched['left_bc']), jnp.asarray(sample_batched['right_bc'])
            ...
            yeex_batch_test = yeex_batch_test.reshape(args.num_GPUs,int(args.batch_size/args.num_GPUs),yeex_batch_test.shape[1],yeex_batch_test.shape[2])
            yeey_batch_test = yeey_batch_test.reshape(args.num_GPUs,int(args.batch_size/args.num_GPUs),yeey_batch_test.shape[1],yeey_batch_test.shape[2])
            ...
            if idx == 0:
                loss, reg_inner, pred_y, bc, grid = eval_step(replicated_models, yeex_batch_test, yeey_batch_test, top_bc_test, bottom_bc_test, left_bc_test, right_bc_test, y_batch_test)
                ...
                plot_helper(yeex_batch_test[0,0,:,:], step, model_path+"/plots/epoch_"+str(step)+"_yeex.png")
            else:
                loss, reg_inner, pred_y, bc, grid = eval_step(replicated_models, yeex_batch_test, yeey_batch_test, top_bc_test, bottom_bc_test, left_bc_test, right_bc_test, y_batch_test)
