...
from JAX_SM_FNO_pml import FNO_multimodal_2d
from simulation_dataset_JAX_pml_only import SimulationDataset
...
@eqx.filter_jit
def Hz_to_Ex_complex(Hz, dL, omega, Sx, Sy, EPSILON_0 = EPSILON_0):
    # yeex => yeex = yeex.type(torch.cdouble) * Sx_f[:, 1:-1]/Sy_f[:, 1:-1]
    yeex = Sx[:, :, 1:-1]/Sy[:, :, 1:-1]
    Ex = 1j*(Hz[:, 1:, 1:-1] - Hz[:, 0:-1, 1:-1])/dL/omega/EPSILON_0/yeex[:, :-1]
    return Ex

@eqx.filter_jit
def Hz_to_Ey_complex(Hz, dL, omega, Sx, Sy, EPSILON_0 = EPSILON_0):
    # yeey = yeey.type(torch.cdouble) * Sy_f[:-2, :]/Sx_f[1:-1, :]
    yeey = Sy[:, 1:-1, :]/Sx[:, 1:-1, :]
    Ey = -1j*(Hz[:,1:-1,1:] - Hz[:,1:-1,:-1])/dL/omega/EPSILON_0/yeey[:, :, :-1]
    return Ey

@eqx.filter_jit
def E_to_Hz_complex(Ey, Ex, Sx, Sy, dL, omega, MU_0 = MU_0):
    Hz = -1j * ((Ey[:, :, 1:] - Ey[:, :, 0:-1]) - (Ex[:, 1:, :] - Ex[:, 0:-1, :]))/dL/omega/MU_0/Sx[:,1:-1,1:-1]/Sy[:,1:-1,1:-1]
    return Hz

@eqx.filter_jit
def H_to_H_pml(Hz_R, Hz_I, dL, omega, Sx, Sy, EPSILON_0 = EPSILON_0, MU_0 = MU_0):
    Sx = jnp.asarray(Sx[:,:,:,0]+1j*Sx[:,:,:,1], dtype=jnp.complex64)
    Sy = jnp.asarray(Sy[:,:,:,0]+1j*Sy[:,:,:,1], dtype=jnp.complex64)
    Hz = jnp.asarray(Hz_R + 1j * Hz_I, dtype=jnp.complex64)

    FD_Ex = Hz_to_Ex_complex(Hz, dL, omega, Sx, Sy, EPSILON_0)
    FD_Ey = Hz_to_Ey_complex(Hz, dL, omega, Sx, Sy, EPSILON_0)
    FD_H = E_to_Hz_complex(FD_Ey, FD_Ex, Sx, Sy, dL, omega, MU_0)

    return jnp.stack((jnp.real(FD_H), jnp.imag(FD_H)), axis = 1)

@eqx.filter_jit
def H_to_bc_pml(Hz_R, Hz_I, dL, omega, EPSILON_0 = EPSILON_0):
    Hz = Hz_R + 1j*Hz_I
...
def main(args,seed):
    ...
    if (args.continue_train == "True"):
        ...
        checkpoint = cloudpickle.loads(pickle.load(_file))
        start_epoch=checkpoint['epoch']
        opt_state = checkpoint['opt_state'],
        optim = checkpoint['optimizer']
        print(f"start_epoch is {start_epoch}")
        model_original = FNO_multimodal_2d(args,key=key)
        model = eqx.tree_deserialise_leaves(model_path+"/last_model.eqx", model_original)
    ...
    def compute_loss_only(model, Sx, Sy, top_bc, bottom_bc, left_bc, right_bc, y):
        pred_y, bc, grid = eqx.filter_vmap(model.eval_forward)(Sx[:,:,:,1], Sy[:,:,:,1], top_bc, bottom_bc, left_bc, right_bc)

        data_loss = MAE_loss(pred_y, y)

        FD_Hy = H_to_H_pml(pred_y[:, :, :, 0], pred_y[:, :, :, 1], dL, omega, Sx, Sy)
    ...
    def compute_loss_and_grad(model, Sx, Sy, top_bc, bottom_bc, left_bc, right_bc, y, dL, omega, reg_norm, data_weight, inner_weight, bc_weight):
        pred_y = eqx.filter_vmap(model)(Sx[:,:,:,1], Sy[:,:,:,1], top_bc, bottom_bc, left_bc, right_bc)
        ...
        # FD_Hy_gt = H_to_H_pml(y[:, :, :, 0], y[:, :, :, 1], dL, omega, Sx, Sy)
        # reg_inner_gt = 1/2 * MAE_loss(FD_Hy_gt[:, 0], y[:, 1:-1, 1:-1, 0]) + 1/2 * MAE_loss(FD_Hy_gt[:, 1], y[:, 1:-1, 1:-1, 1])

        # physical loss for the boundary:
        bc_gt = H_to_bc_pml(y[:,:,:,0], y[:,:,:,1],dL, omega)
        bc_pred = H_to_bc_pml(pred_y[:,:,:,0], pred_y[:,:,:,1], dL,omega)
    ...
    def train_step(rep_models, Sx, Sy, top_bc, bottom_bc, left_bc, right_bc, y, replicated_opt_state, dL, omega, reg_norm, data_weight, inner_weight, bc_weight):
        (loss, aux), grads = compute_loss_and_grad(rep_models, Sx, Sy, top_bc, bottom_bc, left_bc, right_bc, y, dL, omega, reg_norm, data_weight, inner_weight, bc_weight)
    ...
    @eqx.filter_pmap(axis_name='batch')
    def eval_step(rep_models, Sx, Sy, top_bc, bottom_bc, left_bc, right_bc, y):
        loss, reg_inner, pred_y, bc, grid = compute_loss_only(rep_models, Sx, Sy, top_bc, bottom_bc, left_bc, right_bc, y)
    ...
    for step in range(start_epoch, args.epoch):
        ...
        for idx, sample_batched in enumerate(train_loader):
            y_batch_train, Sx_batch_train, Sy_batch_train, top_bc_train, bottom_bc_train, left_bc_train, right_bc_train = jnp.asarray(sample_batched['field']), jnp.asarray(sample_batched['Sx_f']), jnp.asarray(sample_batched['Sy_f']), jnp.asarray(sample_batched['top_bc']), jnp.asarray(sample_batched['bottom_bc']), jnp.asarray(sample_batched['left_bc']), jnp.asarray(sample_batched['right_bc'])
            ...
            Sx_batch_train = Sx_batch_train.reshape(args.num_GPUs,int(args.batch_size/args.num_GPUs),Sx_batch_train.shape[1],Sx_batch_train.shape[2],Sx_batch_train.shape[3])
            Sy_batch_train = Sy_batch_train.reshape(args.num_GPUs,int(args.batch_size/args.num_GPUs),Sy_batch_train.shape[1],Sy_batch_train.shape[2],Sy_batch_train.shape[3])
            ...
            loss, replicated_models, replicated_opt_state, aux = train_step(replicated_models, Sx_batch_train, Sy_batch_train, top_bc_train, bottom_bc_train, left_bc_train, right_bc_train, y_batch_train, replicated_opt_state, dL, omega, reg_norm, args.data_weight, args.inner_weight, args.bc_weight)
        ...
        for idx, sample_batched in enumerate(test_loader):
            y_batch_test, Sx_batch_test, Sy_batch_test, top_bc_test, bottom_bc_test, left_bc_test, right_bc_test = jnp.asarray(sample_batched['field']), jnp.asarray(sample_batched['Sx_f']), jnp.asarray(sample_batched['Sy_f']), jnp.asarray(sample_batched['top_bc']), jnp.asarray(sample_batched['bottom_bc']), jnp.asarray(sample_batched['left_bc']), jnp.asarray(sample_batched['right_bc'])
            ...
            Sx_batch_test = Sx_batch_test.reshape(args.num_GPUs,int(args.batch_size/args.num_GPUs),Sx_batch_test.shape[1],Sx_batch_test.shape[2],Sx_batch_test.shape[3])
            Sy_batch_test = Sy_batch_test.reshape(args.num_GPUs,int(args.batch_size/args.num_GPUs),Sy_batch_test.shape[1],Sy_batch_test.shape[2],Sy_batch_test.shape[3])
            ...
            if idx == 0:
                data_loss, reg_inner, pred_y, bc, grid = eval_step(replicated_models, Sx_batch_test, Sy_batch_test, top_bc_test, bottom_bc_test, left_bc_test, right_bc_test, y_batch_test)
                ...
                plot_helper(Sx_batch_test[0,0,:,:,1], step, model_path+"/plots/epoch_"+str(step)+"_Sx_imag.png")
                plot_helper(Sy_batch_test[0,0,:,:,1], step, model_path+"/plots/epoch_"+str(step)+"_Sy_imag.png")
            else:
                data_loss, reg_inner, pred_y, bc, grid = eval_step(replicated_models, Sx_batch_test, Sy_batch_test, top_bc_test, bottom_bc_test, left_bc_test, right_bc_test, y_batch_test)
