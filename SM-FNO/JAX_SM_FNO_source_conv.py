...
class BasicBlock_no_shortcut(eqx.Module):
    ALPHA: float
    expansion: int
    conv1: eqx.nn.Conv2d
    # bn1: eqx.experimental.BatchNorm
    ...
    # shortcut: eqx.nn.Identity
    def __init__(self, in_planes, planes, ALPHA, stride, *, key):
        super(BasicBlock_no_shortcut, self).__init__()
        keys = jax.random.split(key, num=3)
        self.expansion = 1
        self.ALPHA = ALPHA
        self.conv1 = eqx.nn.Conv2d(in_planes, planes, kernel_size=3, stride=stride, padding=1, use_bias=False, key=keys[0])
...
class FNO_multimodal_2d(eqx.Module):
    ...
    def __init__(self, args, *, key):
        ...
        for i in range(self.num_fourier_layers):
            ...
            self.ws.append(BasicBlock_no_shortcut(self.width, self.width, self.ALPHA, 1, key=keys[10+2*i]))

    @eqx.filter_jit
    def __call__(self, source, top_bc, bottom_bc, left_bc, right_bc):
        ...
        preprocessed = jnp.zeros((2, source.shape[0], source.shape[1]))
        ...
        grid = self.get_grid(source.shape)
        # print("sh:" , source.shape, yeex.shape, yeey.shape, grid.shape, preprocessed.shape)
        # Sx_f = 1/Sx_f
        # Sy_f = 1/Sy_f
        
        pre_data = jnp.concatenate((source.transpose((2,0,1)), grid, preprocessed), axis=0)
        mod_data = pre_data
        # modulating branch:
        ...
        for i in range(self.num_fourier_layers-1):
            ...
            # if self.padding > 0:
            #     inter_x = x[..., :-self.padding, :-self.padding].copy()
            # else:
            #     inter_x = x.copy()

            # inter_x = jax.vmap(jax.vmap(self.fc1, 1, 1),2,2)(inter_x)
            # # x = self.fc1(x)
            # inter_x = jax.nn.leaky_relu(inter_x, negative_slope=self.ALPHA)
            # # inter_x = jax.nn.gelu(inter_x)
            # inter_x = jax.vmap(jax.vmap(self.fc2, 1, 1),2,2)(inter_x)
            # # inter_x = self.fc2(x)
            # inter_x = inter_x.transpose((1, 2, 0))

            # inter_residue = H_to_H(inter_x[None, :, :, 0], inter_x[None, :, :, 1], dL, omega, yeex[None,:,1:-1], yeey[None,1:-1,:])
            # physics_residue = inter_residue[0]-inter_x.transpose((2, 0, 1))[:,1:-1,1:-1]
            # physics_residue = jnp.pad(physics_residue, ((0,0),(1,1),(1,1)), mode='constant', constant_values = 0)
            # if self.padding > 0:
            #     physics_residue = jnp.pad(physics_residue, ((0,0),(0,self.padding),(0,self.padding)), mode='constant', constant_values = 0)
                
            x = jax.nn.leaky_relu(x, negative_slope=self.ALPHA)
            # x.at[-2:,:,:].set(physics_residue)
    ...
    @eqx.filter_jit
    def eval_forward(self, source, top_bc, bottom_bc, left_bc, right_bc):
        # Sx_f: [subdomain_size, subdomain_size,2]
        # Sy_f: [subdomain_size, subdomain_size,2]
        # source: [subdomain_size, subdomain_size, 2]
        # yeex: [subdomain_size, subdomain_size-2]
