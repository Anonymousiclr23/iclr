...
class FNO_multimodal_2d(eqx.Module):
    ...
    @eqx.filter_jit
    def __call__(self, Sx_f_imag, Sy_f_imag, top_bc, bottom_bc, left_bc, right_bc):
        ...
        preprocessed = jnp.zeros((2, Sx_f_imag.shape[0], Sx_f_imag.shape[1]))
        ...
        grid = self.get_grid(Sx_f_imag.shape)
        # print("sh:" , source.shape, yeex.shape, yeey.shape, grid.shape, preprocessed.shape)
        # Sx_f = 1/Sx_f
        # Sy_f = 1/Sy_f
        
        pre_data = jnp.concatenate((jnp.expand_dims(Sx_f_imag,axis=0), jnp.expand_dims(Sy_f_imag,axis=0), grid, preprocessed), axis=0)
        mod_data = pre_data
        # modulating branch:
    ...
    @eqx.filter_jit
    def eval_forward(self, Sx_f_imag, Sy_f_imag, top_bc, bottom_bc, left_bc, right_bc):
        # Sx_f: [subdomain_size, subdomain_size,2]
        # Sy_f: [subdomain_size, subdomain_size,2]
        # source: [subdomain_size, subdomain_size, 2]
        # yeex: [subdomain_size, subdomain_size-2]
